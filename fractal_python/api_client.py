...
class ApiClient(object):
    ...
    def __init__(self, base_url: str, api_key: str, partner_id: str):
        ...
        self.default_headers = {'Content-Type': 'application/json', API_KEY_HEADER: api_key, PARTNER_ID_HEADER: partner_id }

    def call_api(self, resource_path: str, method: str, path_params=None, query_params=None, body=None) -> requests.request:
        if path_params:
            for k, v in path_params:
                resource_path = resource_path.replace("{%s}" % k, quote(str(v)))
        url = self.base_url + resource_path
        return requests.request(method, url, params=query_params, headers=self.default_headers, data=body)
